...
export default function ComplianceReportPage() {
  ...
  const handleExportJson = () => {
    const dataStr = 'data:text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(DEMO_REPORT, null, 2));
    const downloadAnchor = document.createElement('a');
    downloadAnchor.setAttribute('href', dataStr);
    downloadAnchor.setAttribute('download', `parakh-statutory-report-${id}.json`);
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-12 py-2">
      ...
      <div className="no-print p-4 bg-white border border-[#CBD5E1] flex flex-wrap items-center justify-between gap-4 font-mono text-xs shadow-xs">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[#64748B]">
            Inspection Reference: <strong className="text-[#0A2540]">PARAKH/LMPC/2026/00{id}</strong>
          </span>
        </div>
      ...
      {/* Official 12-Section Printable Certificate Document */}
        ...
        <div className="border-b-2 border-[#0A2540] pb-6 flex flex-col md:flex-row items-start md:items-end justify-between gap-4">
          ...
          <div className="text-right font-mono text-xs space-y-1">
            <div>
              ...
              <span className="font-bold text-[#0A2540]">PARAKH/LMPC/2026/00{id}</span>
            </div>
            <div>
              <span className="text-[#64748B]">Inspection Timestamp: </span>
              <span className="text-[#0F172A]">2026-09-04 17:35 IST</span>
            </div>
            <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-mono border border-[#CBD5E1] bg-[#F8FAFC]">
          <div className="p-4 border-b md:border-b-0 md:border-r border-[#CBD5E1] space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <span className="text-[#0F172A] font-bold">SCN-2026-00{id}</span>
              <span className="text-[#64748B]">Inspector:</span>
              <span className="text-[#0F172A]">Field Inspection Officer</span>
          ...
          <div className="p-4 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <span className="text-[#0F172A] font-bold">{DEMO_REPORT.product_name}</span>
              <span className="text-[#64748B]">Category:</span>
              <span className="text-[#0F172A]">{DEMO_REPORT.category}</span>
              <span className="text-[#64748B]">Origin:</span>
              <span className="text-[#0F172A]">{DEMO_REPORT.is_imported ? 'Imported' : 'Domestic (India)'}</span>
              <span className="text-[#64748B]">Packaging Faces:</span>
              <span className="text-[#0F172A]">2 Panels (Front PDP, Back Info)</span>
            </div>
          </div>
        </div>

        {/* 10. Final Legal Verdict Banner */}
        <div className="p-6 border-2 border-[#B91C1C] bg-[#FEF2F2] flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <span className="text-xs font-mono uppercase text-[#B91C1C] font-bold tracking-wider block">
              10. Final Statutory Verdict
            </span>
            <h2 className="text-2xl font-bold text-[#B91C1C] mt-0.5">
              NON-COMPLIANT
            </h2>
            <p className="text-xs text-[#7F1D1D] font-sans mt-1 max-w-md">
              Mandatory retail price declaration under Rule 6(1)(e) is absent. Actionable infraction under Section 36 of Legal Metrology Act, 2009.
            </p>
          </div>

          <div className="grid grid-cols-4 gap-2 text-center font-mono text-xs">
            <div className="p-2.5 bg-white border border-[#FECACA]">
              <span className="text-[10px] text-[#64748B] block">Total</span>
              <span className="font-bold text-sm text-[#0F172A]">{DEMO_REPORT.summary.total_rules}</span>
            </div>
            <div className="p-2.5 bg-white border border-[#FECACA]">
              <span className="text-[10px] text-[#15803D] block">Passed</span>
              <span className="font-bold text-sm text-[#15803D]">{DEMO_REPORT.summary.passed}</span>
            </div>
            <div className="p-2.5 bg-white border border-[#FECACA]">
              <span className="text-[10px] text-[#B91C1C] block">Failed</span>
              <span className="font-bold text-sm text-[#B91C1C]">{DEMO_REPORT.summary.failed}</span>
            </div>
            <div className="p-2.5 bg-white border border-[#FECACA]">
              <span className="text-[10px] text-[#B45309] block">Review</span>
              <span className="font-bold text-sm text-[#B45309]">{DEMO_REPORT.summary.review}</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between font-mono">
            <h3 className="text-xs font-bold uppercase tracking-wider text-[#0A2540]">
              7. Deterministic Rule Evaluation Matrix ({DEMO_REPORT.results.length} Rules)
            </h3>
            <span className="text-xs text-[#B91C1C] font-bold">1 Statutory Violation Detected</span>
          </div>
